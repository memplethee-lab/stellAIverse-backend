...
import { QueryProvenanceDto, ExportProvenanceDto } from './dto/query-provenance.dto';
import {
...
export class ProvenanceController {
  ...
  @ApiOperation({
    ...
    description: 'Get a paginated list of provenance records with optional filters',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Get chronological timeline of provenance records for an agent',
  })
  @ApiParam({ name: 'agentId', description: 'Agent ID', type: 'string' })
  @ApiQuery({
    name: 'fromDate',
  ...
  async getProvenanceTimeline(
    ...
    return this.provenanceService.getProvenanceTimeline(agentId, fromDate, toDate);
  }

  @Get('users/:userId')
  @ApiOperation({
    summary: 'Get provenance for a user',
    description: 'Get all provenance records for a specific user (admin or self only)',
  })
  ...
  @ApiResponse({ status: 403, description: 'Forbidden - can only access own records' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
